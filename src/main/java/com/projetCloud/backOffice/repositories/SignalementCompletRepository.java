...
public interface SignalementCompletRepository extends JpaRepository<SignalementComplet,Long>{
	...
	List<SignalementComplet> findByStatus(final Long idStatus);
	
	@Query(value="SELECT * FROM view_signalementDetails WHERE datesignalement > TO_TIMESTAMP(?1) AND datesignalement < TO_TIMESTAMP(?2)",nativeQuery=true)
	List<SignalementComplet> findBetweenDates(String dateDebut,String dateFin);
